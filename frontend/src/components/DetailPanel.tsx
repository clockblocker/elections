...
export function DetailPanel({ point, detail, loading, error, onClose }: { point: Point | null; detail: PrecinctDetail | null; loading: boolean; error: string | null; onClose(): void }) {
  ...
  return <aside className="detail-panel" aria-live="polite">
    ...
    {detail && <>
      ...
      <Section title="Identity provenance"><Description items={{ "GAS commission ID": detail.gasVyboryId || "Not resolved", "Matching method": detail.matchingMethod ? humanize(detail.matchingMethod) : "No selected match", "Resolution status": detail.gasResolutionStatus ? humanize(detail.gasResolutionStatus) : "Not run", "Resolution reason": detail.gasResolutionReason ? humanize(detail.gasResolutionReason) : "None" }} /></Section>
...
const humanize = (value: string) => value.replace(/_/g, " ").replace(/([A-Z])/g, " $1").replace(/^./, (c) => c.toUpperCase());
