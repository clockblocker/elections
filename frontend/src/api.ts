...
type DetailWire = { result_record_id: number; uik_number: string; gas_vybory_id: string | null; hierarchy: { election: string; ballot: string; region: string; district: string | null; tik: string | null; uik: string }; accounting: Record<string, number | null>; turnout_percent: number | null; party_results: PartyResultWire[]; candidate_results: CandidateResultWire[]; protocols: ProtocolWire[]; commission: { address: string | null; chairperson: MemberWire | null; members: MemberWire[] } | null; match_status: MatchStatus; matching_method: string | null; gas_resolution_status: string | null; gas_resolution_reason: string | null; special_type: string | null; is_deg: boolean; flags: string[]; validation_findings: Array<{ code: string; severity: string; expected: string | null; actual: string | null }>; sources: SourceWire[] };
