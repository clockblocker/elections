...
describe("API wire adapters", () => {
  it("paginates point pages and maps snake_case coordinates", async () => {
    ...
    expect(fetchMock.mock.calls[0][0]).toContain("limit=100000");
    expect(fetchMock.mock.calls[0][0]).toContain("include_total=true");
    expect(fetchMock.mock.calls[1][0]).toContain("offset=100000");
    expect(fetchMock.mock.calls[1][0]).toContain("include_total=false");
  });
