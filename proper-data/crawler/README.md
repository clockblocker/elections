# Official GAS crawler

This directory contains the file-first crawler for recovering official GAS Vybory
result pages. It does not write to the application database. A URL becomes a generated
protocol only after its response has been preserved, checksummed, decoded, and
...
```sh
python3 proper-data/crawler/extract_tree.py \
...
```sh
python3 proper-data/crawler/probe_matrix.py \
...
```sh
python3 proper-data/crawler/cdx_lookup.py \
...
```sh
python3 proper-data/crawler/fetch_cdx.py \
...
```sh
backend/.venv/bin/python proper-data/crawler/decode_result.py page.html \
...
```sh
python3 proper-data/crawler/export_uik_results.py decoded.json \
...
## Current official legacy archive

The current `www.izbirkom.ru` frontend publishes its legacy destination in `env.js`:

```text
LEGACY_IZBIRKOM=http://old.izbirkom.ru
```

Historical result requests that fail on `www.vybory.izbirkom.ru` may therefore still
work on `old.izbirkom.ru`. Current legacy pages obfuscate tables with randomized inline
JavaScript rather than the archived font scheme. Decode a preserved response with:

```sh
backend/.venv/bin/python proper-data/crawler/decode_script_result.py page.html \
  --output reports/generated/gas-archive-lab/decoded.json
```

Generate the checked-in TypeScript protocol tree from reconciled crawler exports:

```sh
python3 proper-data/crawler/generate_typescript.py \
  reports/generated/gas-archive-lab/kerch-all-uik-live-official.json \
